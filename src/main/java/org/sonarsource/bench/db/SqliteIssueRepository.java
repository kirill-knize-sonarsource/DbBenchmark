...
public class SqliteIssueRepository implements IssueRepository {
  ...
  public void insertAll(List<Issue> issues) throws Exception {
    ...
    try (PreparedStatement ps = conn.prepareStatement(sql)) {
      ...
      for (Issue is : issues) {
        ...
        if (is.getLine() == null) ps.setNull(6, Types.INTEGER);
        else ps.setInt(6, is.getLine());
  ...
  private String encodeDetails(Issue is) {
    ...
    // Primary: P|file|sl|el|so|eo|msg\n
    sb.append('P').append('|')
    ...
    // Flows: F;L|file|sl|el|so|eo|msg;L|...\n per flow
    if (is.getFlows() != null) {
    ...
    // Quick fixes: Q;msg|L|file|sl|el|so|eo|msg;L|...\n
  ...
  private void decodeDetails(String details, Issue is) {
    ...
    for (String line : lines) {
      ...
      if (kind == 'P') {
        ...
        if (parts.length >= 7) {
          String file = ub64(parts[1]);
          Integer sl = parseInt(parts[2]);
          Integer el = parseInt(parts[3]);
          Integer so = parseInt(parts[4]);
          Integer eo = parseInt(parts[5]);
          String msg = ub64(parts[6]);
          is.setPrimaryLocation(new IssueLocation(file, sl, el, so, eo, msg));
        }
      } else if (kind == 'F') {
