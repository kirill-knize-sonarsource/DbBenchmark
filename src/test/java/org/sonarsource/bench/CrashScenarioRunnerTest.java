...
class CrashScenarioRunnerTest {
  ...
  @Test
  void crashRunner_full_sqlite_kill_process_during_writes_then_verify_integrity() throws Exception {
    // Create DB path
    File f;
    try {
  ...
  @Test
  void crashRunner_full_h2_kill_process_during_writes_then_verify_integrity() throws Exception {
    // Create DB path
    File f;
    try {
      f = File.createTempFile("h2-crash-full", ".db");
      if (f.exists()) f.delete();
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
    ...
    // Now verify integrity on the same DB path
    String[] verifyArgs = new String[]{"mode=verify", "db=h2", "path=" + f.getAbsolutePath(), "rule=java:S100"};
    assertDoesNotThrow(() -> CrashScenarioRunner.main(verifyArgs));
